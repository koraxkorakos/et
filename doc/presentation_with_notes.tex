...
\note{
- sparse data everywhere

- often compute too much

- goal: compute only needed indices

- quick setup, then core idea
}
\begin{itemize}
...
- AST instead of immediate evaluation

- do not go into internals

- move quickly to projection
}
\begin{lstlisting}
auto expr = a + b - c;
\end{lstlisting}
...
- evaluation as projection onto index set

- evaluate only selected indices

- do not evaluate whole expression
...
\note{
- tree on left, index set on right

- ask only for selected indices

- do not traverse whole tree
...
- dense storage over sparse indices
...
- add: combine left and right at same index

- terminal: value or zero
...
\note{
- now actual work happens

- for each requested index: \lstinline|code|eval at|
...
\note{
- natural support from AST

- union of terminal supports

- evaluate without explicit target set
...
- support of a plus b is union

- projection picks only requested indices
...
- avoids temporary lifetime bugs

- production would use closure strategy
...
\begin{frame}{Summary}
\note{
- expression templates build AST

- projection restricts evaluation

- evaluate only what we need

- this is the one idea to remember
}
\begin{itemize}
...
\begin{frame}{Outlook}
\note{
- future: context, symbolic variables, richer operations

- convolution is easy audience example

- Clifford only if asked

- finish and invite questions
}
\begin{itemize}
